                ...
                <div class="flex flex-col gap-4">
                    <section class="rounded-lg border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-gray-900">
                        ...
                        <div class="grid grid-cols-2 gap-4 max-md:grid-cols-1">
                            <div><label class="mb-1.5 block text-sm font-medium">Menu name (English) *</label><input name="name" required value="{{ old('name', $menu->name) }}" placeholder="Example: Menu One" class="{{ $inputClass }}"></div>
                            <div><label class="mb-1.5 block text-sm font-medium">Menu name (Arabic) <span lang="ar">اسم القائمة</span></label><input dir="rtl" lang="ar" name="name_ar" value="{{ old('name_ar', $menu->name_ar) }}" placeholder="مثال: القائمة الأولى" class="{{ $inputClass }}"></div>
                            <div><label class="mb-1.5 block text-sm font-medium">Client-facing description (English)</label><textarea name="description" rows="3" placeholder="Short description of the menu" class="{{ $inputClass }}">{{ old('description', $menu->description) }}</textarea></div>
                            <div><label class="mb-1.5 block text-sm font-medium">Client-facing description (Arabic) <span lang="ar">الوصف</span></label><textarea dir="rtl" lang="ar" name="description_ar" rows="3" placeholder="وصف مختصر للقائمة" class="{{ $inputClass }}">{{ old('description_ar', $menu->description_ar) }}</textarea></div>
                            ...
                            <div><label class="mb-1.5 block text-sm font-medium">Setup description (English)</label><textarea name="setup_description" rows="3" placeholder="Buffet tables, linens, equipment, setup style…" class="{{ $inputClass }}">{{ old('setup_description', $menu->setup_description) }}</textarea></div>
                            <div><label class="mb-1.5 block text-sm font-medium">Setup description (Arabic) <span lang="ar">وصف التجهيز</span></label><textarea dir="rtl" lang="ar" name="setup_description_ar" rows="3" placeholder="طاولات البوفيه، المفارش، المعدات…" class="{{ $inputClass }}">{{ old('setup_description_ar', $menu->setup_description_ar) }}</textarea></div>
                            <div><label class="mb-1.5 block text-sm font-medium">Service inclusions (English) <span class="font-normal text-gray-500">(one per line)</span></label><textarea name="service_inclusions" rows="4" placeholder="Waiters&#10;Supervisors&#10;Serving equipment" class="{{ $inputClass }}">{{ old('service_inclusions', $menu->service_inclusions) }}</textarea></div>
                            <div><label class="mb-1.5 block text-sm font-medium">Service inclusions (Arabic) <span lang="ar">الخدمات المشمولة</span></label><textarea dir="rtl" lang="ar" name="service_inclusions_ar" rows="4" placeholder="طاقم الخدمة&#10;المشرفون&#10;معدات التقديم" class="{{ $inputClass }}">{{ old('service_inclusions_ar', $menu->service_inclusions_ar) }}</textarea></div>
                        </div>
                    </section>
                        ...
                        <div class="grid grid-cols-2 gap-4 max-lg:grid-cols-1">
                            @foreach ($categories as $category)
                                <div class="rounded-lg border border-gray-200 p-4 dark:border-gray-800">
                                    <div class="mb-3 flex items-center justify-between border-b border-gray-100 pb-2 dark:border-gray-800"><h3 class="font-semibold text-gray-800 dark:text-white">{{ str_pad($loop->iteration, 2, '0', STR_PAD_LEFT) }} · {{ $category->name }} @if ($category->name_ar)<span class="ml-2 font-normal text-gray-500" dir="rtl" lang="ar">{{ $category->name_ar }}</span>@endif</h3><span class="text-xs text-gray-500">{{ $category->products->count() }} available</span></div>
                                    <div class="flex flex-col gap-2">
                                        @forelse ($category->products as $product)
                                            <label class="flex cursor-pointer items-start gap-3 rounded-md p-2 hover:bg-gray-50 dark:hover:bg-gray-950"><input type="checkbox" name="product_ids[]" value="{{ $product->id }}" @checked(in_array((int) $product->id, $selected, true)) class="mt-1 h-4 w-4 accent-brandColor"><span><span class="block text-sm font-medium text-gray-800 dark:text-white">{{ $product->name }}</span>@if ($product->name_ar)<span class="mt-0.5 block text-sm text-gray-600" dir="rtl" lang="ar">{{ $product->name_ar }}</span>@endif @if ($product->description)<span class="mt-0.5 block text-xs text-gray-500">{{ $product->description }}</span>@endif</span></label>
                                        @empty
